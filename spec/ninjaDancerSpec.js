describe("ninjaDancer", function() {

  var ninjaDancer;
  var timeBetweenSteps = 100;
  var clock;

  beforeEach(function() {
    clock = sinon.useFakeTimers();
    ninjaDancer = new NinjaDancer(10, 20, timeBetweenSteps);
  });

  it("should have a jQuery $node object", function(){
    expect(ninjaDancer._$node).to.be.an.instanceof(jQuery);
  });

  xit("should have a step function that makes its node blink", function() {
    sinon.spy(ninjaDancer._$node, 'toggle');
    ninjaDancer.step();
    expect(ninjaDancer._$node.toggle.called).to.be.true;
  });

  describe("dance", function(){
    it("should call step at least once per second", function(){
      sinon.spy(ninjaDancer, "step");
      expect(ninjaDancer.step.callCount).to.be.equal(0);
      clock.tick(timeBetweenSteps); // ? it seems an extra tick is necessary...
      // clock.tick(timeBetweenSteps);

      expect(ninjaDancer.step.callCount).to.be.equal(1);

      clock.tick(timeBetweenSteps);
      expect(ninjaDancer.step.callCount).to.be.equal(2);
    });
  });
});
